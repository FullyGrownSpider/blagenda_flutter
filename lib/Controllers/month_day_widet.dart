...
mixin MonthDayWidget {
  ...
  static const TextStyle smollTextStyleBold =
  ...
  Widget buildMonthDayWidgets(int year, int currentMonth) {
    ...
    for (int i = 0; i < 3; i++) {
      ...
      for (int ii = 0; ii < 4; ii++) {
        ...
        int daysInMonth = DateTimeRange(
                start: monthNow, end: MyDateController(year, nMonth + 1))
            .duration
            .inDays;
        list.add(Text(monthNow.stringMonthSmallDisplay(),
            style: smollTextStyleBold));
        list.add(Text(daysInMonth.toString(),
            style: currentMonth == nMonth
                ? normalTextStyle
                : normalTextStyleBold));
    ...
    return Column(
        ...
        children: rowData
            .map((e) =>
                Row(mainAxisAlignment: MainAxisAlignment.center, children: e)
                    as Widget)
            .toList()
          ..add(smallBlankSplit));
  }
