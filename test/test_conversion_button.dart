...
void deadline() {
  var deadline = Deadline(
      '123', '', 1, Colors.black, MyDateController.nowDate.addOrRemoveDays(10));
  compare(deadline, 6);
}
...
void week() {
  var week = AgainWeekDay('123', '', 1, Colors.black, 1,
      MyDateController.nowDate.addOrRemoveDays(10));
  compare(week, 7);
}

void year() {
  var year = AgainYearDay('123', '', 1, Colors.black, 12, 10,
      MyDateController.nowDate.addOrRemoveDays(10));
  compare(year, 8);
}
...
void compare<t extends BasicButton>(t button, int expected) {
  ...
  // all unique
  expect(
      true, split.where((e) => split.where((ee) => e == ee).isEmpty).isEmpty);
